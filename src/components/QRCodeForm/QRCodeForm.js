...
function QRCodeForm() {
    ...
    const onAddPetBtnClick = async () => {
        ...
        try {
            ...
            const response = await service.savePet({ id: userInfo?.id, pet: formData });
            if (response?._id) {
    ...
    const downloadQRCode = () => {
        ...
        doc.text(qrcodeText, doc.internal.pageSize.getWidth() / 2, 10, { align: 'center' });

        const marginX = (doc.internal.pageSize.getWidth() / 2) - QRSize / 2;
        if (selectedSizeOption === "Default") {
            doc.addImage(pngUrl, "PNG", doc.internal.pageSize.getWidth() / 2 - 20, 20, 40, 40);
                    ...
                    (<div className="qrcode-form-wrapper">
                        <div className="add-pet-form-container">
                            ...
                            <div className="add-pet-form">
                                <input type="file" accept="image/*" placeholder="Pet Image: " onChange={(e) => {
                                    handleFileChange(e.target.files[0])
                                }} />

                                <input type="text" value={petName} placeholder="Pet Name" onChange={(e) => onFieldChange("petName", e.target.value)} />

                                <input type="date" value={petDOB} placeholder="Pet DOB: " min={minDate} onChange={(e) => onFieldChange("petDOB", e.target.value)} />
                                ...
                                {
                                    (errorMsg && !successMsg) && <div className="error">
                                        <p>{errorMsg}</p>
                                    </div>
                                }
                                {
                                    (successMsg && !errorMsg) && <div className="success">
                                        <p>{successMsg}</p>
                                    </div>
                                }
                            </div>
                            ...
                            <div className="qrcode-wrapper">
                                {
                                    QRCodeValue ? (
                                        <div>
                                            <QRCodeCanvas id="qrcode" value={QRCodeValue} size={200} includeMargin={true} />
                                            <div className="select-size-container">
                                                <p>Select size of QR Code below :</p>
                                                <div className="select-size-wrapper">
                                                    <div>
                                                        <input type="radio" value="Default" id="default-size" checked={selectedSizeOption === "Default"} onChange={(e) => setSelectedSizeOption(e.target.value)} />
                                                        <label htmlFor="default-size">Default Size</label>
                                                    </div>

                                                    <div>
                                                        <input type="radio" value="Custom" id="custom-size" checked={selectedSizeOption === "Custom"} onChange={(e) => setSelectedSizeOption(e.target.value)} />
                                                ...
                                                </div>
                                                {
                                                    selectedSizeOption === "Custom" && (
                                                        <div className="qrsize">
                                                            <input type="number" value={QRSize} placeholder="Size" onChange={(e) => handleQRCodeSize(e.target.value)} />
                                                        </div>
                                                    )
                                                }
                                            </div>
                                            <button className="download-qrcode-btn" disabled={!QRCodeValue} onClick={() => downloadQRCode()}>Download QR Code</button>
                                        </div>
